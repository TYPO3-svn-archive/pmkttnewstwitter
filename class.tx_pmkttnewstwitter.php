 ...
 *
 *
 *   58: class tx_pmkttnewstwitter
 *   70:     function processDatamap_afterDatabaseOperations($status, $table, $id, &$fieldArray, &$reference)
 *  213:     function twit_basic($twitter_data)
 *  240:     function twit($twitter_data)
 *  261:     function makeSingleLink()
 *  300:     function init_tmpl($pageId,$template_uid=0)
 *  321:     function getNewsCategory($uid)
 *  344:     function createShortUrl($longURL,$login='',$apiKey='')
 *  378:     function getConfig($pageId)
 *  405:     function isUTF8($str)
 *  415:     function silentDebug($debugdata)
 *
 * TOTAL FUNCTIONS: 10
...
class tx_pmkttnewstwitter {
	...
	/**
	 * Post data on Twitter using Twitter OAuth
	 *
	 * @param	string		$twitter_data: Data to post on twitter.
	 * @return	void
	 */
	...
	function getConfig($pageId) {
		...
		$conf['linkBack'] = $conf['linkBack'] ? 1 : 0;
		...
		$conf['setToNoTweet'] = $conf['setToNoTweet'] ? 1 : 0;
		$conf['doInitialTwitterOAuthOnTestNews'] = $conf['doInitialTwitterOAuthOnTestNews'] ? 1 : 0;
		$conf['twitteraccesstoken'] = trim($conf['twitteraccesstoken']);
		$conf['twitteraccesstokensecret'] = trim($conf['twitteraccesstokensecret']);
		$conf['twitterconsumerkey'] = trim($conf['twitterconsumerkey']);
		$conf['twitterconsumersecret'] = trim($conf['twitterconsumersecret']);
		$conf['twitterdebug'] = $conf['twitterdebug'] ? 1 : 0;
		return $conf;
	}
	 ...
	 * Debug
	 *
	 * @param	string		debug string to output
	 * @return	void
	 */
